...
export const GET = async ({ request }) => {
  ...
  try {
    ...
    if (tokenData.error) {
      ...
      return new Response(`Token error: ${tokenData.error_description}`, {
        status: 400,
      });
    }
    ...
    if (userData.error) {
      ...
      return new Response(`User info error: ${userData.error.message}`, {
        status: 400,
      });
    }

    const { id: google_id, email } = userData;

    // 3. Upsert the user into our SQLite database
    const existingUser = db.prepare(
      "SELECT id, refresh_token FROM users WHERE google_id = ?",
    ).get(google_id);
    ...
    const finalRefreshToken = refresh_token ||
      (existingUser ? existingUser.refresh_token : null);
    ...
    headers.append(
      "Set-Cookie",
      `session_id=${google_id}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${
        60 * 60 * 24 * 30
      }`, // 30 Days
    );
