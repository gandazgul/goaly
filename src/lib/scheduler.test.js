...
Deno.test("scheduleGoal - 3-hour night block does not over-schedule", async () => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(mockEmail, "test_google_id", "token", "refresh", "America/New_York", 21);
  ...
  const goal = db.prepare("SELECT * FROM goals WHERE user_id = ? AND name = ?").get(user.id, "Night Time Gaming");
  ...
  // Assertions
  assert.equal(instances.length, 4, "Should have scheduled exactly 4 instances");
  ...
  for (const instance of instances) {
    ...
    assert.equal(startHour >= 21 || startHour === 0, true, "Should start at or after 9 PM");
  ...
  assert.equal(scheduledDates.size, 4, "Each instance should be scheduled on a different day");
});
...
Deno.test("Timezone string conversions", () => {
  ...
  assert.equal(endOfDay.format("YYYY-MM-DDTHH:mm:ssZ"), "2023-10-05T00:00:00-04:00");
});
