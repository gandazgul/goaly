...
test.describe('Goaly App', () => {
  ...
  test('should display home page and login correctly', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Goaly Logo Goaly' })).toBeVisible();
    ...
    await expect(page.getByText('Logged in as mockuser@example.com')).toBeVisible();
  ...
  test('should allow creating and deleting a new goal', async ({ page }) => {
    ...
    await page.locator('.bg-white', { hasText: testGoalName }).getByRole('button', { name: 'Delete Goal' }).click();
