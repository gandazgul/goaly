...
export const POST = async ({ request, redirect }) => {
  ...
  try {
    ...
    const goal = db.prepare('SELECT id FROM goals WHERE id = ? AND user_id = ?').get(goalId, user.id);
    if (!goal) {
      return new Response('Goal not found or unauthorized', { status: 404 });
    }

    // Fetch all related Google Calendar events for this goal
    const instances = db.prepare('SELECT calendar_event_id FROM goal_instances WHERE goal_id = ? AND calendar_event_id IS NOT NULL').all(goalId);
    ...
    for (const instance of instances) {
      try {
        ...
        console.error(`Failed to delete calendar event ${instance.calendar_event_id}:`, err);
