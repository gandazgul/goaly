import { fetchCalendarEvents, createCalendarEvent } from './calendar.js';
...
export async function scheduleGoal(user, goal) {
  const { id: goalId, times_per_week, duration_minutes, time_preference, color, icon } = goal;
  ...
  for (let i = 0; i < 7; i++) {
    ...
    // Wait, let's just make it simple: schedule at the beginning of free gaps
    let daySlotFound = false;
  ...
  for (const slot of scheduledSlots) {
    ...
    try {
      ...
      console.log(`Scheduled instance for ${goal.name} at ${new Date(slot.start).toLocaleString()}`);
    } catch (err) {
